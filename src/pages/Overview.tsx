...
export function Overview() {
    ...
    () =>
      attractions
        .filter((a) =>
          ...
          ].some((name) => a.name.includes(name.split(" ")[0])),
        )
        .slice(0, 4),
    [attractions],
  );
  ...
  // Hero carousel images from attractions + places
  const defaultHeroImage = "https://images.unsplash.com/photo-1506973035872-a4ec16b8e8d9?w=1400&q=80";
  const heroSlides = useMemo(() => {
    const isHighRes = (url: string) => {
      const w = url.match(/[?&]w=(\d+)/);
      if (w && parseInt(w[1], 10) < 800) return false;
      if (/\bw=\d{1,2}\b/.test(url)) return false;
      return true;
    };
    const slides: { url: string; label: string }[] = [
      { url: defaultHeroImage, label: "Sydney Harbour" },
    ];
  ...
  return (
    <div className="space-y-10 pb-20">
      {/* Hero carousel */}
      <section className="relative overflow-hidden rounded-3xl mx-0 group/hero">
        ...
        {heroSlides.length > 1 && (
          <>
            <button
              type="button"
              onClick={heroPrev}
              className="absolute left-3 top-1/2 -translate-y-1/2 z-10 opacity-0 group-hover/hero:opacity-100 transition-opacity inline-flex h-9 w-9 items-center justify-center rounded-full bg-black/40 backdrop-blur-sm text-white hover:bg-black/60"
            >
            ...
            <button
              type="button"
              onClick={heroNext}
              className="absolute right-3 top-1/2 -translate-y-1/2 z-10 opacity-0 group-hover/hero:opacity-100 transition-opacity inline-flex h-9 w-9 items-center justify-center rounded-full bg-black/40 backdrop-blur-sm text-white hover:bg-black/60"
            >
        ...
        <div className="relative flex min-h-[420px] md:min-h-[480px] flex-col justify-end p-6 md:p-12">
          ...
          <h1 className="text-3xl font-bold tracking-tight text-white sm:text-4xl md:text-6xl font-heading">
            {t("overview.title")}
          </h1>
          ...
          <div className="mt-6 flex flex-wrap gap-3">
            ...
            <Link to="/budget">
              <Button
                size="lg"
                className="rounded-full bg-white/20 text-white border-2 border-white/50 backdrop-blur-sm hover:bg-white/30"
              >
      ...
      <section className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
        <Card className="bg-card shadow-sm border-border/50">
          <CardContent className="flex items-start gap-3 p-4">
            ...
            <div>
              ...
              <p className="font-semibold text-lg font-heading">{tripInfo.dates}</p>
            </div>
          </CardContent>
        </Card>
        <Card className="bg-card shadow-sm border-border/50">
          <CardContent className="flex items-start gap-3 p-4">
            ...
            <div>
              <p className="text-sm text-muted-foreground">
                {t("overview.travelers")}
              </p>
              <p className="font-semibold text-lg font-heading">{tripInfo.travelers.join(", ")}</p>
            </div>
          </CardContent>
        </Card>
        <Card className="bg-card shadow-sm border-border/50">
          <CardContent className="flex items-start gap-3 p-4">
            ...
            <div className="flex-1">
              ...
              <a
                ...
                className="font-semibold text-lg font-heading hover:text-primary inline-flex items-center gap-1 no-underline"
              >
                {tripInfo.hotel.name} <ExternalLink className="h-3 w-3" />
              </a>
              <p className="text-xs text-muted-foreground">
                {tripInfo.hotel.location}
              </p>
            </div>
          </CardContent>
        </Card>
        <Card className="bg-card shadow-sm border-border/50">
          <CardContent className="flex items-start gap-3 p-4">
            ...
            <div>
              <p className="text-sm text-muted-foreground">
                {t("overview.flights")} — {tripInfo.flights.outbound.flightNo}
              </p>
              <div className="flex items-center gap-1.5 text-base font-semibold font-heading">
              ...
              <div className="flex items-center gap-1 text-xs text-muted-foreground mt-0.5">
                ...
                <span>
                  {tripInfo.flights.outbound.duration} direct ·{" "}
      ...
      <section>
        <div className="mb-4 flex items-end justify-between">
          ...
          <Link
            to="/budget"
            className="text-sm font-medium text-primary hover:underline no-underline flex items-center gap-1"
          >
        ...
        <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
          ...
          <Card
            className={cn(
              "border-border/50 shadow-sm sm:col-span-2 lg:col-span-1",
              remaining < 0 && "border-destructive/50",
            )}
          >
            <CardContent className="p-4 space-y-3">
              ...
              <div className="flex justify-between text-sm">
                <span className="text-muted-foreground">
                  {t("overview.spent")}: {formatTWD(totalSpent)}
                </span>
                <span className="text-muted-foreground">
                  {t("budget.totalBudget")}: {formatTWD(totalBudget)}
                </span>
              </div>
              <Progress
                value={
                ...
                <CardContent className="p-3 space-y-1.5">
                  <div className="flex items-center justify-between">
                    ...
                    <span
                      className={cn(
                        "text-xs font-medium",
                        catRemaining < 0 ? "text-destructive" : "text-primary",
                      )}
                    >
                  ...
                  <Progress
                    ...
                    style={
                      {
                        "--progress-color":
                          pct > 90 ? "#ef4444" : CATEGORY_COLORS[b.category],
                      } as React.CSSProperties
                    }
                  />
                  <div className="flex justify-between text-xs text-muted-foreground">
                    <span>
                      {formatTWD(spent)} {t("overview.spent")}
                    </span>
                    <span>
            ...
            {itinerary.map((day, i) => {
              ...
              return (
                <Link
                  key={day.dayLabel}
                  to="/itinerary"
                  className="no-underline snap-start flex-shrink-0 w-[280px] sm:w-[320px]"
                >
      ...
      <section>
        ...
        <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
          {highlights.map((a) => (
            <Card
              key={a.id}
              className="group overflow-hidden border-border/50 shadow-sm hover:shadow-md transition-shadow py-0"
            >
              ...
              >
                {p.image_url ? (
                  <div className="relative h-28 w-full overflow-hidden">
                    ...
                    {p.category && (
                      <Badge className="absolute top-2 left-2 bg-white/90 text-foreground border-0 text-[10px]">
                        {p.category}
                      </Badge>
                    )}
                  </div>
            ...
            <div className="mt-4 flex justify-center">
              <Link to="/places">
                <Button variant="outline" className="rounded-full">
                  {t("overview.seeMore")} (
                  {sortedPlaces.length - PLACES_PREVIEW_LIMIT} more){" "}
                  <ArrowRight className="ml-2 h-4 w-4" />
                </Button>
              </Link>
            </div>
