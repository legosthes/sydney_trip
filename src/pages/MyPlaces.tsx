    ...
    <div className="pb-20 space-y-8">
      ...
      {dialogOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center" role="dialog">
          ...
          <div className="relative z-10 w-full max-w-md mx-4 rounded-xl bg-popover p-6 shadow-xl ring-1 ring-foreground/10 max-h-[90vh] overflow-y-auto" onClick={(e) => e.stopPropagation()}>
            ...
            <form onSubmit={(e) => { e.preventDefault(); handleSubmit(); }} className="space-y-4">
              <div className="space-y-1.5">
                <label htmlFor="place-name" className="text-sm font-medium">{t("places.name")}</label>
                <input id="place-name" type="text" required value={formData.name} onChange={(e) => setFormData({ ...formData, name: e.target.value })} className="flex h-9 w-full rounded-lg border border-input bg-transparent px-3 py-1 text-sm transition-colors placeholder:text-muted-foreground focus:outline-none focus:ring-2 focus:ring-ring" />
              </div>
              <div className="space-y-1.5">
                <label htmlFor="place-cat" className="text-sm font-medium">{t("places.category")}</label>
                <select id="place-cat" value={formData.category} onChange={(e) => setFormData({ ...formData, category: e.target.value })} className="flex h-9 w-full rounded-lg border border-input bg-transparent px-3 py-1 text-sm transition-colors focus:outline-none focus:ring-2 focus:ring-ring">
                  {CATEGORIES.map((c) => <option key={c} value={c}>{c}</option>)}
                </select>
              </div>
              <div className="space-y-1.5">
                <label htmlFor="place-notes" className="text-sm font-medium">{t("places.notes")}</label>
                <textarea id="place-notes" rows={3} value={formData.notes} onChange={(e) => setFormData({ ...formData, notes: e.target.value })} className="flex w-full rounded-lg border border-input bg-transparent px-3 py-2 text-sm transition-colors placeholder:text-muted-foreground focus:outline-none focus:ring-2 focus:ring-ring resize-none" />
              </div>
              <div className="space-y-1.5">
                <label htmlFor="place-maps" className="text-sm font-medium">{t("places.mapsUrl")}</label>
                <input id="place-maps" type="url" placeholder="https://maps.app.goo.gl/..." value={formData.maps_url} onChange={(e) => setFormData({ ...formData, maps_url: e.target.value })} className="flex h-9 w-full rounded-lg border border-input bg-transparent px-3 py-1 text-sm transition-colors placeholder:text-muted-foreground focus:outline-none focus:ring-2 focus:ring-ring" />
              </div>
              <div className="space-y-1.5">
                <label htmlFor="place-web" className="text-sm font-medium">{t("places.website")}</label>
      ...
      {filtered.length === 0 ? (
        <Card className="border-border/50 border-dashed">
          <CardContent className="flex flex-col items-center justify-center py-12 text-center">
            ...
            <p className="font-medium text-muted-foreground">{t("places.noPlaces")}</p>
            <p className="text-sm text-muted-foreground mt-1">{t("places.noPlacesHint")}</p>
          </CardContent>
        </Card>
      ) : (
              ...
              <CardContent className="p-4 space-y-3">
                <div className="flex items-start justify-between gap-2">
                  ...
                  <div className="flex gap-1 flex-shrink-0">
                    <button type="button" className="inline-flex h-7 w-7 items-center justify-center rounded-lg text-muted-foreground hover:text-primary hover:bg-muted transition-colors" onClick={() => openEdit(place)}>
                      <PencilLine className="h-3.5 w-3.5" />
                    </button>
                    <button type="button" className="inline-flex h-7 w-7 items-center justify-center rounded-lg text-muted-foreground hover:text-destructive hover:bg-muted transition-colors" onClick={() => handleDelete(place.id)}>
                ...
                <div className="flex items-center gap-2">
                  {place.maps_url && (
                    <a href={place.maps_url} target="_blank" rel="noopener noreferrer" className="inline-flex items-center gap-1 rounded-full border border-border px-2.5 py-1 text-xs text-foreground hover:bg-secondary transition-colors no-underline">
                      <MapPin className="h-3 w-3" /> {t("places.map")}
                      <ExternalLink className="h-2.5 w-2.5" />
                    </a>
                  )}
                  {place.website && (
                    <a href={place.website} target="_blank" rel="noopener noreferrer" className="inline-flex items-center gap-1 rounded-full border border-border px-2.5 py-1 text-xs text-foreground hover:bg-secondary transition-colors no-underline">
